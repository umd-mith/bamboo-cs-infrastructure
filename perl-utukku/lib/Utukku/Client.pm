package Utukku::Client;
  use Moose;

  use Utukku::Client::Connection;
  use Utukku::Engine::TagLib::Registry;
  use Utukku::Engine::TagLib::Remote;
  use URI;
  use Data::UUID;
  ...
  sub setup {
    ...
    POE::Component::Client::TCP -> new(
      ...
      Connected => sub {
        $self -> connection(Utukku::Client::Connection -> new(
  ...
  sub message {
    ...
    if($msg -> {class} eq 'flow.namespaces.registered') {
      ...
      my $info = $msg -> {data};
      for my $ns (keys %$info) {
        next if Utukku::Engine::TagLib::Registry -> instance -> handler($ns);
        my $h = Utukku::Engine::TagLib::Remote -> new(
          ns => $ns,
          client => $self,
...
Utukku::Client - Scaffolding for building a Utukku client
...
The Utukku::Client uses POE to enable asynchronous interaction with the
