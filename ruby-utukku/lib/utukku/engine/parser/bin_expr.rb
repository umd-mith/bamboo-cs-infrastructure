...
class Utukku::Engine::Parser::BinExpr < Utukku::Engine::Expression
  ...
  def expr_type(context)
    ...
    Utukku::Engine::TagLib.unify_types([ lt, rt ])
  end
  ...
  def run(context, autovivify = false)
    ...
    l.each do |i|
      r.each do |j|
        ut = Utukku::Engine::TagLib.unify_types([ i.vtype, j.vtype ])
        op = Utukku::Engine::TagLib.find_op(ut, self.op)
        if(op && op[:proc])
        ...
        res = res + calc.collect { |c| c.is_a?(Utukku::Engine::Memory::Node) ? c : context.root.anon_node(c, rut) }
      end
    end
    return res
  end
