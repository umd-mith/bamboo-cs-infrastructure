...
class Utukku::Engine::Context
  ...
  def initialize(parent_context = nil, xml = nil)
    ...
    if parent_context.nil?
       if xml.nil? || (xml.root rescue nil).nil?
         roots = { }
         roots['data'] = Utukku::Engine::Memory::Node.new('data', roots, nil, [])
         @root = roots['data']
       end
    end
  ...
  def attribute(ns, attr, popts = { })
    ...
    if !value.nil? && value.is_a?(String)
      ...
      if !e.nil?
        ...
        value = Utukku::Engine::Parser::Literal.new(value, [ Utukku::Engine::NS::FAB, value =~ /^\d+$/ ? 'numeric' : value =~ /^\d*\.\d+$/ || value =~ /^\d+\.\d*$/ ? 'numeric' : 'string' ])
      end
      if opts[:static]
  ...
  def get_select(default = nil)
    self.attribute(Utukku::Engine::NS::FAB, 'select', { :eval => true, :static => false, :default => default })
  end
  ...
  def set_var(v,vv)
    ...
    if @variables.has_key?(v)
      raise Utukku::Engine::Parser::VariableAlreadyDefined.new(v)
    end
    @variables[v] = vv
  end
  ...
  def set_value(p, v)
    ...
    p.each do |pp|
      tgts = pp.is_a?(Utukku::Engine::Memory::Node) ? [ pp ] : pp.run(self, true)
      src = nil
      if !v.nil?
        src = v.is_a?(Utukku::Engine::Memory::Node) ? [ v ] : ( v.is_a?(Array) ? v : v.run(self) )
  ...
  def merge_data(d,p = nil)
    ...
    if d.is_a?(Utukku::Engine::Memory::Node)
      self.set_value(p, d)
    elsif d.is_a?(Array)
      ...
      d.each_pair do |k,v|
        ...
        if v.is_a?(Hash) || v.is_a?(Array) || v.is_a?(Utukku::Engine::Memory::Node)
  ...
  def compile_actions(xml)
    actions = Utukku::Engine::Parser::StatementList.new
    ...
    xml.children.each do |e|
      ...
      ns = e.namespace.href
      next unless Utukku::Engine::TagLib.namespaces.include?(ns)
      if ns == Utukku::Engine::NS::FAB && e.name == 'ensure'
        actions.add_ensure(local_ctx.compile_actions(e))
      elsif ns == Utukku::Engine::NS::FAB && e.name == 'catch'
  ...
  def compile_action(e)
    ns = e.namespace.href
    return unless Utukku::Engine::TagLib.namespaces.has_key?(ns)
    Utukku::Engine::TagLib.namespaces[ns].compile_action(e, self)
  end

  def get_action(ns, nom)
    return unless Utukku::Engine::TagLib.namespaces.include?(ns)
    Utukku::Engine::TagLib.namespaces[ns].get_action(nom, self)
  end

  def action_exists?(ns, nom)
    if ns == Utukku::Engine::NS::FAB
      return true if ['ensure', 'catch'].include?(nom.to_s)
    end

    return false unless Utukku::Engine::TagLib.namespaces.has_key?(ns)
    return Utukku::Engine::TagLib.namespaces[ns].action_exists?(nom)
  end
  ...
  def compile_structural(e)
    ns = e.namespace.href
    return unless Utukku::Engine::TagLib.namespaces.include?(ns)
    Utukku::Engine::TagLib.namespaces[ns].compile_structural(e, self)
  end
  ...
  def run_filter(ns, name)
    handler = Utukku::Engine::TagLib.namespaces[ns]
  ...
  def run_constraint(namespace, name)
    handler = Utukku::Engine::TagLib.namespaces[namespace]
