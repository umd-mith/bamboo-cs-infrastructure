module Utukku::Engine
  module Core
  module Structurals
  class Constraint < Utukku::Engine::Action
    ...
    def compile_xml(xml, ctx)
      ...
      if xml.name == 'value'
        ...
        xml.children do |e|
          ...
          next unless e.namespace.href == Utukku::Engine::NS::FAB
          e_ctx = @context.merge(e)
          case e.name
            when 'param'
              pname = e_ctx.attribute(Utukku::Engine::NS::FAB, 'name')
              if !pname.nil?
                v = e_ctx.attribute(Utukku::Engine::NS::FAB, 'value', { :default => e_ctx.get_select })
