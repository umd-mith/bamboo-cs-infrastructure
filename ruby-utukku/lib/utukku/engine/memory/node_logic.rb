module Utukku::Engine::Memory
  module NodeLogic
      def to_s
        self.to([ Utukku::Engine::NS::FAB, 'string' ]).value
      end
      ...
      def to(t, ctx = nil)
        ...
        path = Utukku::Engine::TagLib.type_path(self.vtype, t)
        return self.anon_node(nil,nil) if path.empty?
        v = self
        ctx = Utukku::Engine::Context.new if ctx.nil?
        path.each do |p|
          vv = nil
          begin
          ...
          if vv.is_a?(Utukku::Engine::Memory::Node)
            v = vv
          else
            v = self.anon_node(vv)
          end
