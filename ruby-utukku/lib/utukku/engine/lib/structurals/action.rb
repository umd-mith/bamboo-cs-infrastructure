...
#
class Utukku::Engine::Lib::Structurals::Action < Utukku::Engine::Structural
    ...
    def run(context, autovivify = false)
      ...
      @context.with(context) do |ctx|
        ...
        if action.has_actions?
          ...
          ctx.set_var('actions', ctx.root.anon_node( @actions, [ Utukku::Engine::NS::FAB, 'expression' ]))
        end
        if action.has_select?
          v = @context.attribute(Utukku::Engine::NS::FAB, 'select', { :eval => true })
  
          ctx.set_var('select', ctx.root.anon_node( v, [ Utukku::Engine::NS::FAB, 'expression' ]))
        end
        ret = action.run(ctx)
      end
      ret
