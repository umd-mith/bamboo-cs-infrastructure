...
module Utukku::Template
  class ParseResult
    ...
    def initialize(text)
      ...
      Utukku::Engine::TagLib.namespaces.each_pair do |ns, ob|
      ...
      @namespaces.keys.each do |p|
        @fab_prefix = p if @namespaces[p] == Utukku::Engine::NS::FAB
      ...
      @doc.root.namespace_definitions.each do |ns|
        if ns.href == Utukku::Engine::NS::FAB
          @fab_ns = ns
        end
      end

      if @fab_ns.nil?
        @fab_ns = Nokogiri::XML::Namespace.new(@doc.root, @fab_prefix, Utukku::Engine::NS::FAB)
      end
    end
    ...
    def to_html(popts = { })
      ...
      deps = { }
      Utukku::Engine::TagLib.namespaces.each_pair do |ns, ob|
        deps[ns] = ob.presentation.get_root_namespaces(:html) & (Utukku::Engine::TagLib.namespaces.keys) - [ ns ]
      end

      ordered_ns = [ ]

      next_round = ([ Utukku::Engine::NS::FAB ] + deps.keys.select { |k| deps[k].empty? }).uniq
      ...
      ordered_ns.each do |ns|
        ob = Utukku::Engine::TagLib.namespaces[ns]
        next if ob.nil?
