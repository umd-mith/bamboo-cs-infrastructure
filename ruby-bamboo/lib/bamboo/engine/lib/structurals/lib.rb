...
class Bamboo::Engine::Lib::Structurals::Lib < Bamboo::Engine::Structural
  ...
  def presentation
    @presentations ||= Bamboo::Engine::TagLib::Presentations.new
  end

  def register_library
    Bamboo::Engine::TagLib.namespaces[self.ns] = self
  end
...
protected
  ...
  def setup(xml)
    xml.attribute_nodes.each do |attr|
      @ns = attr.value if attr.namespace.href == Bamboo::Engine::NS::LIB && attr.name == 'ns'
    end
    #@ns = xml.attribute_with_ns(Bamboo::Engine::NS::LIB, 'ns').value
    Bamboo::Engine::TagLib.namespaces[@ns] = self
