class Bamboo::Engine::Parser::Predicates
      ...
      context.with_roots(possible) do |ctx|
        ...
        if res.is_a?(Array)
          ...
          if !res.empty?
            ...
            unified_type = Bamboo::Engine::TagLib.unify_types(res.collect{ |r| r.vtype })
            case unified_type.join('') 
              when Bamboo::Engine::NS::FAB+'boolean'
                if res.select{ |r| r.to([Bamboo::Engine::NS::FAB, 'boolean']).value }.size > 0
                  n_p << ctx.root
                end
              when Bamboo::Engine::NS::FAB+'numeric'
                if res.select{ |r| r.to([Bamboo::Engine::NS::FAB,'numeric']).value == ctx.position }.size > 0
                  n_p << ctx.root
                end
              when Bamboo::Engine::NS::FAB+'string'
