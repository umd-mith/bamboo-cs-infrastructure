class Bamboo::Engine::Parser::ForExpr
  def initialize(v, e)
    if v.size > 1
      @var = v.shift
      @expr = Bamboo::Engine::Parser::ForExpr.new(v, e)
    else
      @var = v.first
      @expr = e
    end
...
class Bamboo::Engine::Parser::EveryExpr < Bamboo::Engine::Parser::ForExpr
  def expr_type(context)
    [ FAB_NS, 'boolean' ]
  end
...
class Bamboo::Engine::Parser::SomeExpr < Bamboo::Engine::Parser::ForExpr
  def expr_type(context)
    [ FAB_NS, 'boolean' ]
  end
...
class Bamboo::Engine::Parser::ForVar
  def initialize(n, e)
    n =~ /^\$?(.*)$/
    @var_name = $1
    @expr = e
