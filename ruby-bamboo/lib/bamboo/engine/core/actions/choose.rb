module Bamboo::Engine::Core::Actions
  class Choose < Bamboo::Engine::Structural
  ...
  class When < Bamboo::Engine::Action
    namespace Bamboo::Engine::NS::FAB
  ...
  class Block < Bamboo::Engine::Action
  ...
  class Goto < Bamboo::Engine::Action
    namespace Bamboo::Engine::NS::FAB
    attribute :test, :eval => true, :static => false
    attribute :state, :static => true

    def run(context, autovivify = false)
      raise Bamboo::Engine::StateChangeException, @state, caller if @test.nil?
      test_res = @test.run(@context.merge(context)).collect{ |a| !!a.value }
      return [ ] if test_res.nil? || test_res.empty? || !test_res.include?(true)
      raise Bamboo::Engine::StateChangeException, @state, caller
    end
  end

  class Catch < Bamboo::Engine::Action
    namespace Bamboo::Engine::NS::FAB
    attribute :test, :eval => true, :static => false
  ...
  class Raise < Bamboo::Engine::Action
    ...
    def run(context, autovivify = false)
      @context.with(context) do |ctx|
        ...
        raise Bamboo::Engine::Exception.new(res.first)
      end
    end
  end
