module Bamboo::Engine::Core
  require 'bamboo/engine/tag_lib'
  require 'bamboo/engine/core/structurals'
  require 'bamboo/engine/core/actions/choose'
  require 'bamboo/engine/core/actions/for_each'
  require 'bamboo/engine/core/actions/variables'

  class Lib < Bamboo::Engine::TagLib
    ...
    has_type :boolean do
      going_to [ Bamboo::Engine::NS::FAB, 'string' ] do
        weight 1.0
        converting do |i|
          i.root.value ? 'true' : ''
        end
      ...
      going_to [ Bamboo::Engine::NS::FAB, 'numeric' ] do
        weight 1.0
        converting do |i|
    ...
    has_type :string do
      going_to [ Bamboo::Engine::NS::FAB, 'boolean' ] do
        weight 0.0001
        converting do |s|
      ...
      going_to [ Bamboo::Engine::NS::FAB, 'html' ] do
        weight 1.0
        converting do |s|
    ...
    has_type :uri do
      ...
      coming_from [ Bamboo::Engine::NS::FAB, 'string'] do
        weight 1.0
        converting do |u|
          p = u.root.value
          ns = nil
    ...
    NUMERIC = [ Bamboo::Engine::NS::FAB, 'numeric' ]
    ...
    reduction 'sum', { :scaling => :log } do |ctx, args|
      zero = Bamboo::Engine::TagLib.find_op(args.first.vtype, :zero)
      if(zero && zero[:proc])
      ...
      op = Bamboo::Engine::TagLib.find_op(args.first.vtype, :plus)
    ...
    consolidation 'histogram', { :scaling => :log } do |ctx, args|
      ...
      acc.each_pair do |tok, cnt|
        t = ret.create_child(tok, cnt, [Bamboo::Engine::NS::FAB, 'numeric'])
    ...
    STRING = [ Bamboo::Engine::NS::FAB, 'string' ]
    BOOLEAN = [ Bamboo::Engine::NS::FAB, 'boolean' ]
    ...
    function 'true', BOOLEAN do |ctx, args|
      return [ ctx.root.anon_node( true, [ Bamboo::Engine::NS::FAB, 'boolean' ] ) ]
    end

    function 'false', BOOLEAN do |ctx, args|
      return [ ctx.root.anon_node( false, [ Bamboo::Engine::NS::FAB, 'boolean' ] ) ]
    end
    ...
    mapping 'eval' do |ctx, arg|
      p = Bamboo::Engine::Parser.new
      if arg.vtype.join('') == Bamboo::Engine::NS::FAB+'expression'
    ...
    function 'first' do |ctx, args|
      if args.size == 1 && args.vtype.join('') == Bamboo::Engine::NS::FAB + 'tuple'
        args.value.first
      else
        args.first
      end
    ...
    mapping 'uri-prefix' do |ctx, arg|
      ...
      if ctx.get_ns(prefix)
        return ctx.root.anon_node( ctx.get_ns(prefix), [Bamboo::Engine::NS::FAB, 'string'])
      else
        return []
      end
    end
