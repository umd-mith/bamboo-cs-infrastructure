class Bamboo::Engine::TagLib::Type
  def initialize(t)
    @type = t
    @goings = { }
    @comings = { }
  ...
  def going_to(t, &block)
    return unless block
    @goings[t.join('')] ||= [ ]
    obj = Bamboo::Engine::TagLib::TypeConversion.new(t)
...
protected
  ...
  def _unify_types(to, ordered = false)
    ...
    if to.is_a?(Array)
      to = Bamboo::Engine::TagLib.type_handler(to)
      return nil if to.nil?
    end
    ...
    while added
      ...
      [d1, d2].each do |d|
          ...
          if !d[t][:t].nil?
            d[t][:t].outgoing_conversions.each_pair do |conv_key, conv|
              ...
              if d.has_key?(conv_key)
                ...
                added = true
                d[conv_key] = {
                  :t => conv.vtype.is_a?(Array) ? Bamboo::Engine::TagLib.type_handler(conv.vtype) : conv.vtype,
                  :w => w,
        ...
        Bamboo::Engine::TagLib.types.keys.each do |ns|
          Bamboo::Engine::TagLib.types[ns].each_pair do |ct, tob|
            to_key = ns + ct.to_s
            tob.incoming_conversions.each_pair do |from_key, conv|
              ...
              if d.has_key?(to_key)
                ...
                added = true
                d[to_key] = {
                  :t => Bamboo::Engine::TagLib.type_handler([ ns, ct ]),
                  :w => w * 95.0 / 100.0,
...
class Bamboo::Engine::TagLib::TypeConversion
  def initialize(t)
    @type = t
    @weight = 0.0
     @guard = nil
