package Bamboo::Shell;
  ...
  has '_parser' => ( accessor => 'parser', is => 'rw', default => sub { Bamboo::Engine::Parser -> new } );
  has '_context' => ( accessor => 'context', is => 'rw', default => sub { Bamboo::Engine::Context -> new } );
  ...
  sub shell {
    Bamboo::Shell -> new_with_options() -> run();
  }
  ...
  sub run {
    ...
    if( $self -> suppress_readline ) {
      ...
      $self -> term(Term::ReadLine -> new("Bamboo Shell"))
        if( ! $self -> term
    ...
    unless( $self -> suppress_narrative ) {
      $self -> print("\nbamboo shell -- Bamboo (v$Bamboo::VERSION)\n");
    ...
    eval {
      require Bamboo::Shell::Base;
      #Bamboo::Shell::Base -> init_commands($self);
    };

    $self -> find_commands('Bamboo::Shell');
  ...
  sub immediate {
    ...
    if( $self -> handlers -> {$bits[0]} ) {
      ...
      Bamboo::Shell::Base -> instance -> immediate($self, @bits);
    }
  }
